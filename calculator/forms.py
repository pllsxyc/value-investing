...
class DcfCalculationForm(forms.ModelForm):
    tags = forms.ModelMultipleChoiceField(
        queryset=DcfTag.objects.none(),
        widget=forms.CheckboxSelectMultiple,
        required=False,
        label='分类标签',
        help_text='可多选已创建的标签，收藏会归类显示在左侧的每个所选标签下。',
    )
    ...
    class Meta:
        ...
        help_texts = {
            ...
            'current_fcf': '最近一年 FCFF，公司自由现金流，通常为 EBIT×(1-税率)+折旧摊销-资本开支-营运资本增加。单位：万元。',
            ...
            'net_debt': '有息债务减现金，计算股权价值时从企业价值中扣除。单位：万元。',
            'non_operating_assets': '不参与主营经营但归属股东的资产，计算股权价值时加回。单位：万元。',
            'shares_outstanding': '总股本，建议使用稀释后股本，用于计算每股价值。单位：万股（与上面的万元相约，每股价值即为元）。',
        }
        widgets = {
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        if not tags.exists():
            self.fields['tags'].help_text = '还没有标签，请先在上方创建标签。'
    ...
    def clean(self):
        ...
        if self.user and self.user.is_authenticated and not tags:
            self.add_error('tags', '请先创建并选择至少一个标签。')
